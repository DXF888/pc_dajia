$(function() {
	...
	$(window).scroll(function() {
		...
		sidNav_roll($sidNav,wind_scrollTop,$sideNav_li);
		function sidNav_roll(nav,scrollTop,nav_li){
			//判断侧边导航是否显示
			if(nav.css('display') == 'block') {	
				...
				if(scrollTop >= getModuleTop('#Wedding-Hotel')) {
					nav_li.removeClass('active');
				...
				if(scrollTop >= getModuleTop('#Wedding-Dress')) {
					nav_li.removeClass('active');
					nav_li.eq(3).addClass('active');
				}
				if(scrollTop >= getModuleTop('#Service-Personal')) {
					nav_li.removeClass('active');
					nav_li.eq(4).addClass('active');
				}
				if(scrollTop >= getModuleTop('#Wedding-Company')) {
					$sideNav_li.removeClass('active');
					$sideNav_li.eq(5).addClass('active');
				}
				if(scrollTop >= getModuleTop('#Wedding-Dessert')) {
					$sideNav_li.removeClass('active');
					$sideNav_li.eq(6).addClass('active');
				}
				if(scrollTop >= getModuleTop('#Wedding-Car')) {
					$sideNav_li.removeClass('active');
					$sideNav_li.eq(7).addClass('active');
				}
			}
		}
